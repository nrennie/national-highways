...
daily_report <- function(
  ...
  api_call <- glue::glue(
    "{url}/reports/daily?sites={site}&start_date={start_date}&end_date={end_date}&page={page}&page_size={page_size}" # nolint
  )
  ...
  output <- api_df$Rows |>
    ...
    dplyr::mutate(
      dplyr::across(
        .data$`Time Interval`:.data$`Total Volume`, as.numeric
      )
    )
  return(output)
}
